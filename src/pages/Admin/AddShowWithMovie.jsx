...
const [theatres,setTheatres] = useState([])
    ...
    const { GETMOVIES_API,GETTHEATRES_API} = movieAPI;
...
function changeHr(e){
  if(e.target.value == -1){
    ...
    async function submitHandler(){
      ...
      if(res.data.success == false){
        ...
        if(selectedtheatre == selectedtheatreForListing)
  ...
  return (
    <div className={'w-[95%] md:w-[70%] bg-bg-4 flex flex-col flex-wrap p-2 rounded-lg mt-2 mx-auto '+ (small && "  text-[3vw]")}>
      
<div className='flex text-white w-full justify-center flex-wrap'>
                <div className='flex justify-start items-center w-full flex-wrap sm:min-w-72 sm:w-[50%]'>
            <p htmlFor='theatre' className='text-textcolor'>Movie : </p>
            <div>
    <select
        ...
        <div className='flex justify-center w-full flex-wrap  my-2'>
...
export default AddShowWithMovie
