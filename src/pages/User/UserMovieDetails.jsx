...
const UserMovieDetails = () => {
  ...
  return loading ? (
    ...
    <div
      className={`min-h-[100vh] bg-bg-1 w-[100vw] justify-center flex flex-wrap flex-col items-center text-white`}
    >
      <div className="sm:w-[70%] w-[90%]   mt-24  ">
        <div>
          <div className={"flex flex-wrap    gap-7 " +(small && " justify-center w-full")}>

            <img
              src={movie?.image}
              className="w-52 h-80 rounded-md  shadow-2xl"
            />
            <div className={"py-10 " + (small && " w-full")}>
        ...
        <div className="mt-9 sm:ml-3 flex justify-between w-[70%] items-center flex-wrap">
          <p className="text-textcolor text-[6vw] sm:text-3xl font-semibold">Showtimes</p>
            ...
            <ShowList twoDshows={twoDshows} setAddShow={setAddShow} selectedtheatre={selectedtheatre} setTheatre={setTheatre}  settwoDShows={settwoDShows} setThreeDShows={setThreeDShows} threeDshows={threeDshows}/>
