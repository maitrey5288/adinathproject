...
const Card = ({movie}) => {
  ...
  return (
    <div onClick={()=>navigate(`/movieDetails/${movie._id}`)}  onMouseOver={handleMouseOver}
    onMouseOut={handleMouseOut} className={` bg-bg-2   card w-[200px]   hover:shadow-md  border border-textcolorb  cursor-pointer  rounded-lg   flex flex-col justify-start`}>
          ...
          <div className='absolute top-0 w-full  h-full flex items-center justify-center'>
            ...
            <div className='text-gray-300'>{new Date(movie.releaseDate).toISOString().split('T')[0]}</div>
